...
- **SKILL-TREE**: 디버깅, 파이프라인 운영, 멱등성
- **발견 문제**:
  1. PK 중복 크래시 → `insert_or_ignore` 구현 (멱등성 확보)
  2. commits↔PRs JOIN 불가 → `author_login` 필드 추가, staging에서 `COALESCE(login, name)`
- **산출물**: 멱등성 구현 + 식별자 통합 + 멀티 repo 테스트 (pallets/flask 200건)
- **학습 포인트**: 식별자 불일치는 데이터 통합의 핵심 문제, raw에서 가능한 많은 정보 수집
- [x] 완료 (2026-03-22)
